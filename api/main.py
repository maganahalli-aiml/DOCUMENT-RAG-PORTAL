...
async def compare_documents(reference: UploadFile = File(...), actual: UploadFile = File(...)) -> Any:
    try:
        ...
        # Convert DataFrame to records format expected by frontend
        rows = df.to_dict(orient="records")
        
        # Ensure the keys match what the frontend expects (Page -> Page, changes -> Changes)
        formatted_rows = []
        for row in rows:
        ...
        return {"rows": formatted_rows, "session_id": dc.session_id}
    except HTTPException:
        raise
    except Exception as e:
